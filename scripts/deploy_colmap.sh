...
tar -czf /tmp/${LOT_NAME}_${SIZE}.tar.gz -C $DATA_DIR .
wait

echo "Launching vast.ai job..."
echo '#!/bin/bash
echo "Instance started - waiting for data..."
sleep 10
' > /tmp/onstart.sh
...
scp -r utils/colmap_utils.py utils/colmap_process.py dataset/colmap_manager_cloud.py root@$INSTANCE_ID.inst.vast.ai:/workspace/

echo "Running COLMAP on instance..."
ssh root@$INSTANCE_ID.inst.vast.ai << EOF
  cd /workspace
  echo "Extracting data..."
  tar -xzf ${LOT_NAME}_${SIZE}.tar.gz
  echo "Running COLMAP..."
  uv run python colmap_manager_cloud.py
